...
class FixtureResearchProvider(BaseResearchProvider):
    ...
    def _run_ai4si_research(self, problem_spec: ProblemSpec) -> ResearchEvidenceResult:
        ...
        if domain == "rmab":
            ...
            cards = [
                MethodEvidenceCard(method_family="distributionally_robust_optimization", archetypes_supported=["robust_preference_learning"], assumptions=["Uncertainty set can be specified (e.g., KL or Wasserstein ball)", "Worst-case distribution is a meaningful adversary"], data_requirements=["preference samples or outcome data", "uncertainty quantification"], constraint_support="native via adversarial constraints", fairness_implications=["Protects against worst-case harm to subgroups"], implementation_burden="high", interpretability="medium", failure_modes=["Overly conservative if the uncertainty set is too wide"], baseline=False, confidence="high", citations=[Citation(source_id="ai4si-dro-1", url="ai4si://dro/dpo_pro", title="Distributionally Robust Optimization", source_type="ai4si")]),
                MethodEvidenceCard(method_family="direct_preference_optimization", archetypes_supported=["robust_preference_learning"], assumptions=["Pairwise preference data is available"], data_requirements=["preference pairs"], constraint_support="external", fairness_implications=["Preference noise can encode or amplify bias"], implementation_burden="medium", interpretability="medium", failure_modes=["Sensitive to preference labeling noise and distribution shift"], baseline=True, non_ml_alternative=False, confidence="high", citations=[Citation(source_id="ai4si-dro-2", url="ai4si://dro/dpo", title="Direct Preference Optimization", source_type="ai4si")]),
                MethodEvidenceCard(method_family="stackelberg_games", archetypes_supported=["robust_preference_learning", "game_theoretic"], assumptions=["Adversary or uncertainty can be modeled as a best-responding player"], data_requirements=["utility or payoff estimates", "adversary behavior model"], constraint_support="native", fairness_implications=["Can encode fairness in the leader objective"], implementation_burden="high", interpretability="high", failure_modes=["Mis-specifying the adversary model leads to poor robustness"], baseline=False, confidence="medium", citations=[Citation(source_id="ai4si-dro-3", url="ai4si://dro/stackelberg_robust", title="Robust Stackelberg", source_type="ai4si")]),
            ]
            return ResearchEvidenceResult(search_queries=["distributionally robust optimization preference", "DPO robust fine-tuning uncertainty", "worst-case robust security optimization"], candidate_method_families=[c.method_family for c in cards], evidence_cards=cards)

        elif domain == "bandits":
            cards = [
                MethodEvidenceCard(method_family="contextual_bandits", archetypes_supported=["sequential_decision"], assumptions=["Context features are observable and informative", "Rewards are roughly stationary"], data_requirements=["context vectors, historical feedback"], constraint_support="external control", fairness_implications=["Context features can embed or amplify bias"], implementation_burden="medium", interpretability="medium", failure_modes=["Non-stationary rewards undermine regret bounds"], baseline=True, non_ml_alternative=False, confidence="high", citations=[Citation(source_id="ai4si-bandit-1", url="ai4si://bandit/contextual", title="Contextual Bandits", source_type="ai4si")]),
                MethodEvidenceCard(method_family="thompson_sampling", archetypes_supported=["sequential_decision"], assumptions=["Prior over reward distributions is tractable", "Posterior updates are feasible"], data_requirements=["reward observations over time"], constraint_support="external", fairness_implications=["Posterior can reflect observed bias"], implementation_burden="medium", interpretability="medium", failure_modes=["Poor prior choice slows convergence"], baseline=False, confidence="high", citations=[Citation(source_id="ai4si-bandit-2", url="ai4si://bandit/thompson", title="Thompson Sampling", source_type="ai4si")]),
                MethodEvidenceCard(method_family="upper_confidence_bound", archetypes_supported=["sequential_decision"], assumptions=["Rewards are bounded", "Finite action space"], data_requirements=["reward observations per arm"], constraint_support="external", fairness_implications=["Uniform exploration ignores subgroup-specific needs"], implementation_burden="low", interpretability="high", failure_modes=["Over-exploration can be costly in sensitive contexts"], baseline=True, non_ml_alternative=False, confidence="high", citations=[Citation(source_id="ai4si-bandit-3", url="ai4si://bandit/ucb", title="UCB Algorithm", source_type="ai4si")]),
            ]
            return ResearchEvidenceResult(search_queries=["multi-armed bandit exploration exploitation", "thompson sampling contextual bandit", "UCB regret bound"], candidate_method_families=[c.method_family for c in cards], evidence_cards=cards)

        elif domain == "pomdp":
            cards = [
                MethodEvidenceCard(method_family="pomdp_planning", archetypes_supported=["partially_observable"], assumptions=["State is partially observable via an observation model", "Belief can be updated from observations"], data_requirements=["transition and observation probabilities or learned model"], constraint_support="via belief-state planner", fairness_implications=["Belief updates can propagate measurement bias"], implementation_burden="high", interpretability="medium", failure_modes=["Belief-space explosion in large state spaces"], baseline=False, confidence="high", citations=[Citation(source_id="ai4si-pomdp-1", url="ai4si://pomdp/planning", title="POMDP Planning", source_type="ai4si")]),
                MethodEvidenceCard(method_family="belief_state_rl", archetypes_supported=["partially_observable"], assumptions=["Deep function approximator can learn useful belief features"], data_requirements=["interaction traces or simulator"], constraint_support="external", fairness_implications=["Harder to audit belief representations"], implementation_burden="very_high", interpretability="low", failure_modes=["Unstable training with sparse observations"], baseline=False, confidence="medium", citations=[Citation(source_id="ai4si-pomdp-2", url="ai4si://pomdp/belief_rl", title="Belief-State RL", source_type="ai4si")]),
                MethodEvidenceCard(method_family="heuristic_search", archetypes_supported=["partially_observable"], assumptions=["Problem admits a compact heuristic evaluation"], data_requirements=["state and action encodings"], constraint_support="native", fairness_implications=["Heuristic choices can encode bias"], implementation_burden="medium", interpretability="high", failure_modes=["Heuristic mis-specification yields suboptimal plans"], baseline=True, non_ml_alternative=True, confidence="medium", citations=[Citation(source_id="ai4si-pomdp-3", url="ai4si://pomdp/heuristic", title="Heuristic POMDP Search", source_type="ai4si")]),
            ]
            return ResearchEvidenceResult(search_queries=["POMDP planning belief state", "partially observable information gathering", "belief-space RL"], candidate_method_families=[c.method_family for c in cards], evidence_cards=cards)
    ...
    def _build_queries(self, problem_spec: ProblemSpec) -> list[str]:
        ...
        if problem_spec.domain_hint:
            ...
            if domain == "rmab":
                ...
                queries.insert(0, "distributionally robust optimization preference uncertainty")
                queries.insert(1, "DPO robust fine-tuning worst-case social impact")
            elif domain == "bandits":
                queries.insert(0, "multi-armed bandit exploration exploitation social impact")
                queries.insert(1, "contextual bandit thompson sampling UCB public health")
            elif domain == "pomdp":
                queries.insert(0, "POMDP planning belief state information gathering")
                queries.insert(1, "partially observable Markov decision social network")
